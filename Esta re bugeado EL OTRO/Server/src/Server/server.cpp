...
void* server::procesar(void)
{
	...
	while(this->isRunning())
	{
		...
		if (m_queue.size() != 0)
		{
			...
			// BLOQUE DE PROCESAMIENTO
			for(int pos=0;pos<m_listaDeClientes.size();pos++)
				if(m_listaDeClientes.isAvailable(pos))
					m_queuePost[pos].add(serverMsg);
...
bool server::procesarMensaje(ServerMessage* serverMsg)
{
	...
	if ((netMsg.msg_Code[0] == 'm') && (netMsg.msg_Code[1] == 'v') && (netMsg.msg_Code[2] == 'x'))
	{
		std::cout << "mvx!\n";
		//procesamientoExitoso = (StringHelper::validateDouble(stringValue));
		procesamientoExitoso = true;
	}
	if ((netMsg.msg_Code[0] == 'm') && (netMsg.msg_Code[1] == 'v') && (netMsg.msg_Code[2] == 'y'))
	{
		std::cout << "mvy!\n";
		procesamientoExitoso = (StringHelper::validateDouble(stringValue));
	}
